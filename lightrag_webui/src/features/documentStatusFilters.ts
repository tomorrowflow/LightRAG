...
const ANALYZING_STATUS_FILTERS: DocStatus[] = ['preprocessed', 'parsing', 'analyzing', 'ready', 'handling']
...
export const getStatusBucket = (status: DocStatus): StatusBucket => {
  ...
  return status as Exclude<DocStatus, 'parsing' | 'analyzing' | 'preprocessed' | 'ready' | 'handling'>
}

export const getGroupedStatusesForFilter = (statusFilter: StatusFilter): DocStatus[] | null => {
  switch (statusFilter) {
    case 'analyzing':
      return ANALYZING_STATUS_FILTERS  // preprocessed, parsing, analyzing, ready, handling
    case 'processed':
    case 'processing':
    case 'pending':
    case 'failed':
